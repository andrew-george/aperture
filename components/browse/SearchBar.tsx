...
function SearchBar() {
  ...
  const { activeResultIndex, handleKeyDown, setActiveResultIndex } = useAccessibleResultsMenu(searchResults, inputRef)
  ...
  useEffect(() => {
    if (debouncedSearchTerm.length === 0) {
      ...
      fetchSearchResults(debouncedSearchTerm).then(items => setSearchResults(items))
    }
                ...
                return (
                  <li
                    ...
                    <div className="relative w-10 h-10">
                      <Image src={img} alt={slug} fill className="object-cover rounded-sm" />
                    </div>
